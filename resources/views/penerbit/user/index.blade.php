            ...
            <div class="bg-[#FAFBFC] overflow-hidden shadow-sm sm:rounded-lg">
                <div class="text-gray-900 px-64">
                    <div class="container py-20 space-y-14">
                        ...
                        @if ($penerbit->count() === 0)
                            <p class="text-center">Belum ada penerbit tersedia</p>
                        @endif
                        @foreach ($penerbit as $pen)
                            <div class="bg-white shadow-[0_4px_4px_0_rgba(0,0,0,0.25)] rounded-sm p-3 space-y-6">
                                <div class="w-full space-y-3">
                                    <h1 class="text-2xl">{{ $pen->nama }}</h1>
                                    @foreach ($pen->buku as $buku)
                                        <div class="bg-gray-100 p-3 rounded-lg shadow-inner">
                                            <h2 class="text-xl">{{ $buku->judul_buku }}</h2>
                                            <p class="text-sm">Pengarang: {{ $buku->pengarang }}</p>
                                            <p class="text-sm">Penerbit: {{ $pen->nama }}</p>
                                            <p class="text-sm">Ketersediaan: {{ $buku->ketersediaan ?: 0 }}</p>

...
<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-2">
            <div class="bg-white/40 text-sm backdrop-blur-sm border border-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-16 text-gray-900 h-full">
                    <h1 class="text-center bg-gradient-to-r from-cyan-900 to-cyan-500 text-transparent bg-clip-text text-3xl font-serif font-bold pb-8">
                        History
                    </h1>
                    ...
                    @foreach ($transaksi->unique('nomor_peminjaman') as $t)
                        ...
                        <div x-data="{
                            ...
                            <div @click="setTimeout(() => handleClick(), delay)"
                                ...
                                <div class="items-center py-1 flex justify-between grow px-8">
                                    ...
                                    <div class="text-center">
                                        ...
                                        <p>{{ Carbon::parse($t->peminjaman->tanggal_peminjaman)->addDays(3)->format('d-m-Y') }}</p>
                                    </div>
                            ...
                            <div x-ref="content" :style="{ height: `${height}px` }" :class="`mb-2 overflow-hidden transition-height duration-300 ease-in-out `">

                                @foreach ($transaksi as $item)
                                    @if ($t->nomor_peminjaman === $item->nomor_peminjaman)
                                        @php
                                            $tanggal = $item->tanggal_pengembalian ? Carbon::parse($item->tanggal_pengembalian) : now();
                                            $durasi = $item->peminjaman?->durasi?->durasi ?? 0; // Default to 0 if durasi is null
                                            $tanggalPeminjaman = Carbon::parse($item->peminjaman->tanggal_peminjaman);
                                            $dueDate = $tanggalPeminjaman->addDays($durasi)->startOfDay();
                                            $daysDifference = $tanggal->startOfDay()->diffInDays($dueDate, false); // Get signed diff
                                            $absoluteDaysDifference = abs($daysDifference); // Get absolute value
                                            $finePerDay = $item->peminjaman?->denda?->denda ?? 3000; // Default fine per day
                                            $fine = $tanggal->greaterThan($dueDate) ? $absoluteDaysDifference * $finePerDay : 0;
                                            
                                            $totalDenda += $fine; // Tambahkan denda ke total denda
                                        @endphp
                                        <div class="">
                                            <div class="bg-slate-200 grid grid-cols-8 items-center gap-8 border-0 font-mono border-cyan-700 border-t shadow-md rounded-l-3xl rounded-br-[30px] cursor-pointer">
                                                ...
                                                <div class="px-10 justify-center items-center">
                                                    @switch($item->status)
                                                        @case(0)
                                                            <p class="bg-gradient-to-r from-yellow-400 to-yellow-600 rounded-xl w-fit px-2 py-1 text-white">
                                                                Pending
                                                            </p>
                                                        @break

                                                        @case(1)
                                                            <p class="bg-gradient-to-r from-green-400 to-green-600 rounded-xl w-fit px-2 py-1 text-white">
                                                                Borrowing
                                                            </p>
                                                        @break

                                                        @case(2)
                                                            <p class="bg-gradient-to-r from-red-400 to-red-600 rounded-xl w-fit px-2 py-1 text-white">
                                                                Rejected
                                                            </p>
                                                        @break

                                                        @case(3)
                                                            <p class="bg-gradient-to-r from-orange-400 to-orange-600 rounded-xl w-fit px-2 py-1 text-white">
                                                                Payment Required
                                                            </p>
                                                        @break

                                                        @case(4)
                                                            <p class="bg-gradient-to-r from-blue-400 to-blue-600 rounded-xl w-fit px-2 py-1 text-white">
                                                                Complete
                                                            </p>
                                                        @break

                                                        @default
                                                            <p class="bg-gradient-to-r from-gray-400 to-gray-600 rounded-xl w-fit px-2 py-1 text-white">
                                                                Unknown
                                                            </p>
                                                    @endswitch
                                                </div>
                                                <div class="col-span-2 text-center">
                                                    ...
                                                    <h1>{{ $item->tanggal_pengembalian ? Carbon::parse($item->tanggal_pengembalian)->locale('id')->format('d-m-Y') : 'Belum Dikembalikan' }}</h1>
                                                </div>
                                                <div class="text-center">
                                                ...
                                                <div class="flex h-16 rounded-3xl items-center justify-center justify-self-end">
                                                    <form action="{{ route('pinjaman.finish', ['nomor_peminjaman' => $item->nomor_peminjaman, 'nomor_buku' => $item->nomor_buku]) }}" class="h-full" method="POST">
                                                        @csrf
                                                        @method('PATCH')
                                                        <button class="text-center relative h-full align-middle bg-gradient-to-b cursor-pointer disabled:cursor-auto text-white px-3 py-1 rounded-br-3xl disabled:from-gray-400 disabled:to-salte-400" type="submit" {{ $item->status !== 3 ? 'disabled' : '' }}>
